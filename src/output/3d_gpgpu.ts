...
import {Vec2, GPGPU, Geometry, Mesh3d, Layer3d, Renderer3d} from 'sprite-extend-3d';
...
function updateMouse(e: any) {
  ...
  const renderer = layer.renderer as Renderer3d;

  // Get mouse value in -1 to 1 range, with y flipped
  mouse.value.set(
    (e.x / renderer.width) * 2 - 1,
    (1.0 - e.y / renderer.height) * 2 - 1
  );
}
