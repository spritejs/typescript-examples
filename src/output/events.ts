...
(async function () {
  ...
  (fglayer.canvas as HTMLCanvasElement).style.backgroundColor = '#F0DBAE';
  ...
  function setKey(key: string, x: number, y: number) {
  ...
  let moving: spritejs.Animation|null = null;
  function moveY(destY: number) {
    flapping.pause();
